...
sapply(seq(gc$url), function (x) { # loop to load tables in BQ project dataset from GC
  ...
  system(paste0("bq --location=EU load --replace --source_format=CSV --autodetect --null_marker='NA' --field_delimiter=", delimiter, " ", dataset_name, ".", tbl, " ", url))
    
  } else {
    
    system(paste0("bq --location=EU load --replace --source_format=CSV --null_marker='NA' --field_delimiter=", delimiter, " ", dataset_name, ".", tbl, " ", url, " ./", schema))
