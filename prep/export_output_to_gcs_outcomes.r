...
export_output_to_gcs_outcomes <- function () {
...
#                    priority = "BATCH")
  ...
  dest_tbls <- paste0(outcome_gc_bucket, tbl, ".csv")
  ...
  # print("updated tables sent to GCS")
    ...
    sapply(seq(tbl2$table_id), function (x) {
      ...
      dest_tbls <- paste0(outcome_gc_bucket, tbl, "*.csv") # using wildcards to export > 1GB files
