...
input_gc_bucket <- "gs://genetics-portal-ukbb-mr-eur/protein-gwas/SHIN/harmonised" # Path to input files in a google bucket

output_gc_bucket <- "gs://genetics-portal-ukbb-mr-eur/protein-gwas/SHIN/pan_filtered/" # Path to google bucket where exposure output files will be stored

outcome_gc_bucket <- "gs://genetics-portal-ukbb-mr-eur/protein-gwas/SHIN/pan_outcomes_gsmr_filtered/" # store the filtered exposure-SNP matched outcomes here
...
schema_n <- "TotalSampleSize" # Name of sample size column in schema (for case-control studies, this will be the total sample size) (Note: one can also provide the sample size directly if they are uniform across tables)
...
dataset_name <- "shin" # Name of dataset that will be created in BigQuery
