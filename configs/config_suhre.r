...
input_gc_bucket <- "gs://genetics-portal-ukbb-mr-eur/protein-gwas/SUHRE/harmonised_removedcol17" # Path to input files in a google bucket

output_gc_bucket <- "gs://genetics-portal-ukbb-mr-eur/protein-gwas/SUHRE/pan_filtered/" # Path to google bucket where exposure output files will be stored

outcome_gc_bucket <- "gs://genetics-portal-ukbb-mr-eur/protein-gwas/SUHRE/pan_outcomes_gsmr_filtered/" # store the filtered exposure-SNP matched outcomes here
...
schema_se <- "abs(hm_beta/z)" # Name of standard error column in schema (if absent, use z-statistic and beta)

schema_p <- "p_value" # Name of p-value column in schema

schema_n <- "neff" # Name of sample size column in schema (for case-control studies, this will be the total sample size) (Note: one can also provide the sample size directly if they are uniform across tables)
...
dataset_name <- "suhre" # Name of dataset that will be created in BigQuery
